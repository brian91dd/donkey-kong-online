...
*******************************************************************************/
cPlayer::cPlayer( char player, string name, int x, int y ) : player_number(player), wins(0), lives(3), x_start(x), y_start(y), speed_y(0)
{
  this->name = name;
...
void cPlayer::restart()
{
  switch( this->player_number )
  {
    case cPlayer::P1:
    {
      this->player = &this->player_standby_right;
      this->direction = DIR_RIGHT;
      break;
    }
    case cPlayer::P2:
    {
      this->player = &this->player_standby_left;
...
/******************************************************************************
* Mueve la rana	(RANA? JAJAJAJAJ RE QUEMADO) en la posicion indicada ( MOVE_UP, MOVE_DW, etc )
*******************************************************************************/
void cPlayer::update( map<string,bool> key, SDL_Surface* mask )
{
  ...
  switch( this->status )
  {
    case cPlayer::DEAD:
    {
      if( !this->mask_color( "UNDER", C_FLOOR ) && !this->mask_color( "UNDER", C_LADDER_TOP ) ) this->y = this->y + 1;
      this->player->animate( false );
      if( this->hold_tick + 3500 < SDL_GetTicks() ) this->restart();
      break;
    }
    ...
    case cPlayer::ALIVE:
    {
      ...
      if( !key["RIGHT"] && key["LEFT"] ) this->direction = DIR_LEFT;

      //printf( "Doing: %d / Speed: %f / (%d,%d) / H:(%d,%d,%d) / F:(%d,%d,%d) / U:(%d,%d,%d) \n", this->doing, this->speed_y, this->x, this->y, this->colors[0].r, this->colors[0].g, this->colors[0].b, this->colors[1].r, this->colors[1].g, this->colors[1].b, this->colors[4].r, this->colors[4].g, this->colors[4].b );
      ...
      {
        // Fix: Si esta escalando y se quedo si escalera por debajo
...
void cPlayer::die()
{
  ...
  this->player->restart_animation();
  this->hold_tick = SDL_GetTicks();
  this->lives--;
}

/******************************************************************************
* Se ejecuta cuando la rana llega al nido
*******************************************************************************/
void cPlayer::win()
{
  /*
  this->status = cPlayer::ALIVE_WIN;
  this->frog = this->frog_win;
  this->hold_tick = SDL_GetTicks();
  this->wins++;
  */
}
...
/******************************************************************************
* Setea la posicion (x,y) pasada por parametro jajaaja
*******************************************************************************/
