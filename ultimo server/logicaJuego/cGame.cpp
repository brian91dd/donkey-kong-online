...
tick_time(0),player1_name(player1_name), payer2_name(player2_name)
{
...
void cGame::new_game( int difficulty )
{
  ...
  // Creo los players
  this->p1 = new cPlayer( cPlayer::P1, this->player1_name, 359, 536 );
...
void cGame::run()
{
 ...
 //Mensaje: NEW_GAME PLAYER_1_NAME;PLAYER_2_NAME;VIDAS_INICIALES;
 //X_DK_1;Y_DK_1;DIRECTION_DK_1;X_DK_2;Y_DK_2;DIRECTION_DK_2;X_OIL_1;Y_OIL_1;DIRECTION_OIL_1;X_OIL_2;Y_OIL_2;DIRECTION_OIL_2;X_PAULINE;Y_PAULINE|

	char mensajeEnviarJug1[500];
	char mensajeEnviarJug2[500];

	sprintf(mensajeEnviarJug1,"%d %s;%s;%s;",NEW_GAME,this->p1->name,this->p2->name,this->max_lives);
	sprintf(mensajeEnviarJug2,"%d %s;%s;%s;",NEW_GAME,this->p2->name,this->p1->name,this->max_lives);
	...
	map<int,cDonkeyKong*>::iterator p = this->obj_donkeykongs.begin();
	  while( p != this->obj_donkeykongs.end() )
	  {
		...
		sprintf(aux,"%d;%d;%d;",donkeykong->x,donkeykong->y,donkeykong->direction);
		strcat(subMsg,aux);
		p++;
		free(aux);
	  }
	...
	map<int,cOil*>::iterator p = this->obj_oils.begin();
	  while( p != this->obj_oils.end() )
	  {
		char aux[500];
		cOil* oil = p->second;
		sprintf(aux,"%d;%d;%d;",oil->x,oil->y,oil->direction);
		strcat(oilsMsg,aux);
		p++;
		free(aux);
	  }

	strcat(subMsg,oilsMsg);

	sprintf(subMsg,"%s%d;%d|",subMsg,this->pauline->x,this->pauline->y);

	strcat(mensajeEnviarJug1,subMsg);
	strcat(mensajeEnviarJug2,subMsg);

	send(mensajeEnviarJug1); //ENVIAR A CADA CLIENTE POR SU RESPECTIVO SOCKET - BOCETO
	send(mensajeEnviarJug2);

	this->running = false;

	free(mensajeEnviarJug1);
	free(mensajeEnviarJug2);
  ...
  while( this->running )
  {
	  ...
	  char mensajeEnviarJug2[500];
	   ...
	   //Mensaje: UPDATE_MAP P1_X;P1_Y;P1_STATUS;P1_DOING;P1_DIRECTION;P2_X;P2_Y;P2_STATUS;P2_DOING;P2_DIRECTION;TYPE_BARRIL,X,Y,DOING;TYPE_FLAME,X,Y|
//Por ejemplo, si fuera un barril sería ;B,342,453,2; si fuera una llama quedaría ;F,343,123;.
//Solo se deben enviar las posiciones de los barriles y llamas con status = STATUS_ACTIVE.

	sprintf(mensajeEnviarJug1,"%d %d;%d;%d;%d;%d;%d;%d;%d;%d;%d;",
						UPDATE_MAP,this->p1->x,this->p1->y,this->p1->status,this->p1->doing,this->p1->direction,
								   this->p2->x,this->p2->y,this->p2->status,this->p2->doing,this->p2->direction);

	sprintf(mensajeEnviarJug2,"%d %d;%d;%d;%d;%d;%d;%d;%d;%d;%d;",
					UPDATE_MAP,this->p2->x,this->p2->y,this->p2->status,this->p2->doing,this->p2->direction,
							   this->p1->x,this->p1->y,this->p1->status,this->p1->doing,this->p1->direction);
	  ...
	  while( p != this->obj_donkeykongs.end() )
	  {
		  ...
		  while( pb != donkeykong->obj_barrels.end() )
		  {
			...
			{
				char aux[500];
			    sprintf(aux,"%c,%d,%d,%d;",cGame::BARREL,barrel->x,barrel->y,barrel->doing);
			    strcat(barrelsMsg,aux);
				free(aux);
			 }
		    ++pb;
		   }
	...
	strcat(mensajeEnviarJug1,barrelsMsg);
	strcat(mensajeEnviarJug2,barrelsMsg);
	  ...
	  while( p != this->obj_oils.end() )
	  {
		  ...
		  list<cFlame*>::iterator pb = oil->obj_flames.begin();
		  while( pb != oil->obj_flames.end() )
		  {
			cFlame* flame = *pb;
			if( flame->status == cFlame::STATUS_ACTIVE )
			{
				char aux[500];
			    sprintf(aux,"%c,%d,%d;",cGame::FLAME,flame->x,flame->y);
			    strcat(flamesMsg,aux);
				free(aux);
			 }
			 ++pb;
		   }
	...
	strcat(mensajeEnviarJug1,flamesMsg);
	strcat(mensajeEnviarJug2,flamesMsg);
...
void cGame::update_playing()
{
  ...
  this->p2->update( this->key, this->level_mask );
...
void cGame::render_playing()
{
  ...
  if( this->p2->lives > 0 )
  {
    ...
    {
      this->dstrect.x = this->w - 155 + 48 + 12 * i;
      this->dstrect.y = 24;
      SDL_BlitSurface( this->img_life, 0, this->screen, &this->dstrect );
    }
  }
  ...
  if( this->p2->wins > 0 )
  {
    ...
    {
      this->dstrect.x = this->w - 135 + 48 + 12 * i;
      this->dstrect.y = 35;
      SDL_BlitSurface( this->img_pauline, 0, this->screen, &this->dstrect );
    }
  }
  ...
  // Textos
  this->writer.printf( 5, 5, "Nombre: " + this->p1->name, 14, 0xFFFFFF );
  this->writer.printf( 5, 17, "Vidas:", 14, 0xFFFFFF );
  this->writer.printf( 5, 29, "Paulines:", 14, 0xFFFFFF );
  this->writer.printf( this->w - 150, 5, "Nombre: " + this->p2->name, 14, 0xFFFFFF );
  this->writer.printf( this->w - 150, 17, "Vidas:", 14, 0xFFFFFF );
  this->writer.printf( this->w - 150, 29, "Paulines:", 14, 0xFFFFFF );
