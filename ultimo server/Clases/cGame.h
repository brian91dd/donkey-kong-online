...
{
  public:
    cGame( std::string player1_name, std::string player2_name );
    void run();
    ~cGame();
  ...
  //private:
    ...
    std::map<std::string,bool> key1;
    std::map<std::string,bool> key2;
    ...
    std::string send_new_game();
    std::string send_update_map();
};
