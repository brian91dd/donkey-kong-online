...
*******************************************************************************/
cPlayer::cPlayer( char player, string name, int x, int y, int lives ) : player_number(player), name(name), wins(0), lives(lives), x_start(x), y_start(y), speed_y(0), player(NULL), god_mode_tick(-5000)
{
  // Cargo las imagenes
...
void cPlayer::die()
{
  if( this->god_mode_tick + cPlayer::GOD_MODE < (int) SDL_GetTicks() )
  {
...
void cPlayer::render( SDL_Surface* screen )
{
  ...
  switch( this->status )
  {
    ...
    case cPlayer::ALIVE:
    {
      // Si la imagen actual es de muerto o victoria, pongo el god mode
      if( this->player == &this->player_dead || this->player == &this->player_win ) this->god_mode_tick = SDL_GetTicks();
